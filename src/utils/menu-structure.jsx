...
export function Clickable({ className, item }) {
  console.log(item)
  return <div className={className}>      {item.title ? item.title : startCase(camelCase(item.item))}
  </div>
}
...
export function MenuItemTag({ item, className }) {
  return (
    // to={`/tags/${item.item.toLowerCase()}`}
    <Clickable className={className} item={item}>
      {item.title ? item.title : startCase(camelCase(item.item))}
    </Clickable>
  )
}
